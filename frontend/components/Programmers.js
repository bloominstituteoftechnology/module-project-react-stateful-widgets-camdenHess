...
export default function Programmers() {
  ...
  const [selectedProgrammerId, setSelectedProgrammerId] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.find(programmer => programmer.id === selectedProgrammerId).name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: selectedProgrammerId ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {
                setSelectedProgrammerId(dev.id)
              }}>Feature</button>
            </div>
          )
        }
